...
class MaintainanceAddNoteController extends Controller {
    ...
    public function index(){
        ...
        $data['attributes'] = $this->get_maintainance_add_note_attributes(NULL, NULL);
        return view('maintainance.notes.maintainance_add_note')->with('MAN', $data);
    }

    private function get_maintainance_add_note_attributes($process, $request){
    ...
    public function maintainance_add_process(Request $request){
        ...
        if($validation_result['validation_result'] == TRUE){
            ...
            $data['attributes'] = $this->get_maintainance_add_note_attributes($saving_process_result, $request);

        }else{

            $validation_result['process_status'] = FALSE;

			$data['attributes'] = $this->get_maintainance_add_note_attributes($validation_result, $request);
        }
    ...
    private function validation($request){
        ...
        try{
            ...
            $rules['category'] = array('required', new ZeroValidation('Category', $request->category));
    ...
    private function saving_process($request){
        ...
        try{
            ...
            $data['maintainance_add_note'] = $this->get_maintainance_add_notes($request);
            ...
            $saving_result = $objMaintainanceAddProcess->save_maintainance_add_notes($data);
    ...
    private function get_maintainance_add_notes($request){
        ...
        foreach($terminal_detail as $terminal){
            ...
            //$maintainance_add_note[$icount]['MAN_id'] = '';

            $maintainance_add_note[$icount]['ma_date'] = date('Y/m/d');
            $maintainance_add_note[$icount]['bank'] = $request->bank;
            $maintainance_add_note[$icount]['bank_ma_id'] = 0;
            $maintainance_add_note[$icount]['maintainance_category_id'] = $request->category;
            $maintainance_add_note[$icount]['referance_number'] = ltrim(rtrim($terminal));
            $maintainance_add_note[$icount]['remark'] = $request->remark;
            $maintainance_add_note[$icount]['saved_by'] = Auth::user()->id;
            $maintainance_add_note[$icount]['saved_on'] = now();
            $maintainance_add_note[$icount]['cancel'] = 0;
            $maintainance_add_note[$icount]['cancel_by'] = NULL;
            $maintainance_add_note[$icount]['cancel_on'] = NULL;
            $maintainance_add_note[$icount]['cancel_reason'] = NULL;

            $icount ++;
        }

        return $maintainance_add_note;
    }
    ...
    private function get_terminal_log($request){
        ...
        foreach($terminal_detail as $terminal){
            ...
            $terminal_log[$icount]['maintain'] = 1;
