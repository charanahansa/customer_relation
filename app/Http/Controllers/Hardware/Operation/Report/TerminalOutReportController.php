...
use App\Models\Hardware\Operation\TerminalOutProcess;

use PhpOffice\PhpSpreadsheet\Spreadsheet;
use PhpOffice\PhpSpreadsheet\Writer\Xlsx;
use PhpOffice\PhpSpreadsheet\Style\Border;

class TerminalOutReportController extends Controller {
    ...
    public function index(){
        ...
        return view('hardware.operation.report.terminal_out_report')->with('top', $data);
    }

    public function terminal_out_report_process(Request $request){

		$objTerminalInProcess = new TerminalOutProcess();
		...
		if( isset($input['from_jobcardno']) && isset($input['to_jobcardno']) ){
			...
			$jobcard_no_filter = " && jobcard_no between '". $input['from_jobcardno'] ."' and '". $input['to_jobcardno'] ."'  ";
		}
		...
		$total_filter = $serial_no_filter . $jobcard_no_filter . $bank_filter . $model_filter;
	...
	private function genarate_excel_file($resultset){
        ...
        $sheet->setCellValue('F1', 'Released Date');
        $sheet->setCellValue('G1', 'Released By');
		$sheet->setCellValue('H1', 'Released For');
		...
		$sheet->getStyle('A1:I1')->getFill()->setFillType(\PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID)->getStartColor()->setARGB('49fc03');
		...
		$spreadsheet->getActiveSheet()->getStyle('A1:I1')->getBorders()->applyFromArray($border_styleArray);
		...
		foreach($resultset as $row){
            ...
            $sheet->setCellValue('F' . $icount, $row->R_Date);
            $sheet->setCellValue('G' . $icount, $row->name);
            $sheet->setCellValue('H' . $icount, $row->R_To);
            $sheet->setCellValue('I' . $icount, $row->remark);
		...
		$spreadsheet->getActiveSheet()->getStyle('A2:I'.$icount)->getBorders()->applyFromArray($border_styleArray);

		$sheet->getStyle('A:I')->getAlignment()->setHorizontal('left');
