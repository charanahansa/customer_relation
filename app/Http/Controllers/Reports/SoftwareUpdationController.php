...
use App\Models\Reports\SoftwareUpdation;

use PhpOffice\PhpSpreadsheet\Spreadsheet;
use PhpOffice\PhpSpreadsheet\Writer\Xlsx;
use PhpOffice\PhpSpreadsheet\Style\Border;

class SoftwareUpdationController extends Controller {
    ...
    public function index(){
		...
		$data['sub_status'] = $objSoftwareUpdation->getSubStatus();
		$data['status'] = $objSoftwareUpdation->getStatus();

        return view('reports.software_updation_report')->with('SU', $data);
    }

    public function software_updation_report_process(Request $request){
		...
		if(isset($input['bank'])){
			...
			$bank_filter = " && su.bank in (". $bank_list .") ";
		...
		if(isset($input['model'])){
			...
			$model_filter = " && sud.model in (". $model_list .") ";
		}
		...
		if(isset($input['officer'])){
			...
			$officer_filter = " && sud.officer in (". $officer_list .") ";
		}
		...
		if(isset($input['sub_status'])){
			...
			$sub_status_filter = " && sud.sub_status in (". $sub_status_list .") ";
		}
		...
		if(isset($input['status'])){
			...
			$status_filter = " && sud.status in (". $status_list .") ";
		}
		...
		$result = $objSoftwareUpdation->get_report($input['from_date'], $input['to_date'], $total_filter, $tables);
	...
	private function genarate_excel_file($resultset, $tables){
		...
		*/
		...
		$filename = 'Software_Updating_Report';
