...
use App\Models\Reports\TerminalReplacement;

use PhpOffice\PhpSpreadsheet\Spreadsheet;
use PhpOffice\PhpSpreadsheet\Writer\Xlsx;
use PhpOffice\PhpSpreadsheet\Style\Border;

class TerminalReplacementController extends Controller {
    ...
    public function index(){
		...
		$data['bank'] = $objTerminalReplacement->get_bank();
		$data['model'] = $objTerminalReplacement->get_models();
		$data['officers'] = $objTerminalReplacement->get_officers();
		$data['sub_status'] = $objTerminalReplacement->get_sub_status();
		$data['status'] = $objTerminalReplacement->get_status();

        return view('reports.terminal_replacement_report')->with('TR', $data);
    }

    public function terminal_replacement_report_process(Request $request){
		...
		$based_tid_filter = '';
		$replaced_tid_filter = '';
		$based_serialno_filter = '';
		$replaced_serialno_filter = '';
		$merchant_filter = '';
		$contactno_filter = '';
		$sim_no_filter = '';
		$pod_no_filter = '';
		...
		if( isset($input['from_based_tid']) && isset($input['to_based_tid']) ){
			...
			$based_tid_filter = " && tr.based_tid between '". $input['from_based_tid'] ."' and '". $input['to_based_tid'] ."'  ";
		}

		// Based Serial No.
		if( isset($input['from_based_serialno']) && isset($input['to_based_serialno']) ){

			$serialno_filter = " && tr.based_serialno between '". $input['from_based_serialno'] ."' and '". $input['to_based_serialno'] ."'  ";
		}

		// Replaced Tid
		if( isset($input['from_replaced_tid']) && isset($input['to_replaced_tid']) ){

			$replaced_tid_filter = " && tr.replaced_tid between '". $input['from_replaced_tid'] ."' and '". $input['to_replaced_tid'] ."'  ";
		}

		// Replaced Serial No.
		if( isset($input['from_replaced_serialno']) && isset($input['to_replaced_serialno']) ){

			$serialno_filter = " && tr.replaced_serialno between '". $input['from_replaced_serialno'] ."' and '". $input['to_replaced_serialno'] ."'  ";
		}
		...
		if(isset($input['bank'])){
			...
			$bank_filter = " && tr.bank in (". $bank_list .") ";
		...
		if(isset($input['model'])){
			...
			$model_filter = " && tr.based_model in (". $model_list .") ";
		}
		...
		if(isset($input['model'])){
			...
			$model_list = substr(rtrim($model_list),0,-1);
			$model_filter = " && tr.replaced_model in (". $model_list .") ";
		}
		...
		if(isset($input['officer'])){
			...
			$officer_filter = " && tr.officer in (". $officer_list .") ";
		}
		...
		if(isset($input['sub_status'])){
			...
			$sub_status_filter = " && tr.sub_status in (". $sub_status_list .") ";
		}
		...
		if(isset($input['status'])){
			...
			$status_filter = " && tr.status in (". $status_list .") ";
		}

		// Tables
		$tables = $input['tables'];

		$total_filter = $based_tid_filter . $replaced_tid_filter;
		$total_filter .= $based_serialno_filter . $replaced_serialno_filter;
		$total_filter .= $based_model_filter . $replaced_model_filter;
		$total_filter .= $bank_filter . $officer_filter . $sub_status_filter . $status_filter;

		$result = $objTerminalReplacement->get_report($input['from_date'], $input['to_date'], $total_filter, $tables);
	...
	private function genarate_excel_file($resultset, $tables){
		...
		$sheet->setCellValue($this->getExcelColumn($col_count) . '2', 'Category'); $col_count++;
		$sheet->setCellValue($this->getExcelColumn($col_count) . '2', 'Based Ref No.'); $col_count++;
		$sheet->setCellValue($this->getExcelColumn($col_count) . '2', 'Based Tid'); $col_count++;
		$sheet->setCellValue($this->getExcelColumn($col_count) . '2', 'Based Model'); $col_count++;
		$sheet->setCellValue($this->getExcelColumn($col_count) . '2', 'Based Serial No.'); $col_count++;
		$sheet->setCellValue($this->getExcelColumn($col_count) . '2', 'Replaced Ref No.'); $col_count++;
		$sheet->setCellValue($this->getExcelColumn($col_count) . '2', 'Replaced Tid'); $col_count++;
		$sheet->setCellValue($this->getExcelColumn($col_count) . '2', 'Replaced Model'); $col_count++;
		...
		if(in_array("terminal_replacement_fs_view", $tables)){
			...
			$sheet->setCellValue($this->getExcelColumn($col_count) . '2', 'FS Based Tid'); $col_count++;
			$sheet->setCellValue($this->getExcelColumn($col_count) . '2', 'FS Based Model'); $col_count++;
			$sheet->setCellValue($this->getExcelColumn($col_count) . '2', 'FS Based Serial No.'); $col_count++;
			$sheet->setCellValue($this->getExcelColumn($col_count) . '2', 'FS Replaced Tid'); $col_count++;
			$sheet->setCellValue($this->getExcelColumn($col_count) . '2', 'FS Replaced Model'); $col_count++;
			$sheet->setCellValue($this->getExcelColumn($col_count) . '2', 'FS Replaced Serial No.'); $col_count++;
			$sheet->setCellValue($this->getExcelColumn($col_count) . '2', 'FS Merchant'); $col_count++;
		...
		foreach($resultset as $row){
			...
			$sheet->setCellValue($this->getExcelColumn($col_count) . $icount, $row->category);  $col_count++;
			$sheet->setCellValue($this->getExcelColumn($col_count) . $icount, $row->based_ref_no);  $col_count++;
			$sheet->setCellValue($this->getExcelColumn($col_count) . $icount, $row->based_tid);  $col_count++;
			$sheet->setCellValue($this->getExcelColumn($col_count) . $icount, $row->based_model);  $col_count++;;
			$sheet->setCellValue($this->getExcelColumn($col_count) . $icount, $row->based_serialno);  $col_count++;
			$sheet->setCellValue($this->getExcelColumn($col_count) . $icount, $row->replaced_ref_no);  $col_count++;
			$sheet->setCellValue($this->getExcelColumn($col_count) . $icount, $row->replaced_tid);  $col_count++;
			...
			if(in_array("terminal_replacement_ftl_view", $tables)){
				...
				$sheet->setCellValue($this->getExcelColumn($col_count) . $icount, $row->ftl_substatus);  $col_count++;
			...
			if(in_array("terminal_replacement_fs_view", $tables)){
				...
				$sheet->setCellValue($this->getExcelColumn($col_count) . $icount, $row->fs_based_tid);  $col_count++;
				$sheet->setCellValue($this->getExcelColumn($col_count) . $icount, $row->fs_based_model);  $col_count++;
				$sheet->setCellValue($this->getExcelColumn($col_count) . $icount, $row->fs_based_serialno);  $col_count++;
				$sheet->setCellValue($this->getExcelColumn($col_count) . $icount, $row->fs_replaced_tid);  $col_count++;
				$sheet->setCellValue($this->getExcelColumn($col_count) . $icount, $row->fs_replaced_model);  $col_count++;
				$sheet->setCellValue($this->getExcelColumn($col_count) . $icount, $row->fs_replaced_serialno);  $col_count++;
				$sheet->setCellValue($this->getExcelColumn($col_count) . $icount, $row->fs_merchant);  $col_count++;
		...
		$sheet->setCellValue('E1', $col_count);
		$sheet->setCellValue('F1', $icount);
		...
		if(in_array("terminal_replacement_tmc_view", $tables)){
			...
			$spreadsheet->setActiveSheetIndex(0)->getColumnDimension('BA')->setAutoSize(true);
			...
			$spreadsheet->setActiveSheetIndex(0)->getColumnDimension('BD')->setAutoSize(true);
		}
		...
		if(in_array("terminal_replacement_tp_view", $tables)){
			...
			$spreadsheet->setActiveSheetIndex(0)->getColumnDimension('BF')->setAutoSize(true);
		...
		$filename = 'Terminal_Replacement_Report';
