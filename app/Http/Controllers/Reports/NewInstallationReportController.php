...
use App\Models\Reports\New_Install;
...
class NewInstallationReportController extends Controller {
	...
	public function index(){
        ...
        $objNew = new New_Install();

		$data['bank'] = $objNew->get_bank();
		$data['model'] = $objNew->get_models();
		$data['officers'] = $objNew->get_officers();
		$data['sub_status'] = $objNew->get_sub_status();
		$data['status'] = $objNew->get_status();

        return view('reports.new_installation_report')->with('New', $data);
    }

	public function new_installation_report_process(Request $request){
		...
		if( isset($input['from_tid']) && isset($input['to_tid']) ){
			...
			$tid_filter = " && n.tid between '". $input['from_tid'] ."' and '". $input['to_tid'] ."'  ";
		}

		// Serial No.
		if( isset($input['from_serialno']) && isset($input['to_serialno']) ){

			$serialno_filter = " && n.serialno between '". $input['from_serialno'] ."' and '". $input['to_serialno'] ."'  ";
		}
		...
		if(isset($input['bank'])){
			...
			$bank_filter = " && n.bank in (". $bank_list .") ";
		...
		if(isset($input['model'])){
			...
			$model_filter = " && n.model in (". $model_list .") ";
		}
		...
		if(isset($input['officer'])){
			...
			$officer_filter = " && n.officer in (". $officer_list .") ";
		}
		...
		if(isset($input['sub_status'])){
			...
			$sub_status_filter = " && n.sub_status in (". $sub_status_list .") ";
		}
		...
		if(isset($input['status'])){
			...
			$status_filter = " && n.status in (". $status_list .") ";
		}
		...
		$result = $objNew->get_report($input['from_date'], $input['to_date'], $total_filter, $tables);
	...
	private function genarate_excel_file($resultset, $tables){
		...
		$sheet->setCellValue($this->getExcelColumn($col_count) . '2', 'Replacement Type'); $col_count++;
		...
		foreach($resultset as $row){
			...
			$sheet->setCellValue($this->getExcelColumn($col_count) . $icount, $row->replacement_type);  $col_count++;
			$sheet->setCellValue($this->getExcelColumn($col_count) . $icount, $row->replacement_no);  $col_count++;
		...
		$filename = 'NewInstallation_Report';
