...
use App\Models\Reports\Breakdown;
...
class BreakdownReportController extends Controller{
	...
	public function index(){
		...
		$data['bank'] = $objBreakdown->get_bank();
		$data['model'] = $objBreakdown->get_models();
		$data['fault'] = $objBreakdown->get_faults();
		$data['action_taken'] = $objBreakdown->get_action_taken();
		$data['relevant_detail'] = $objBreakdown->get_relevant_details();
		$data['officers'] = $objBreakdown->get_officers();
		$data['sub_status'] = $objBreakdown->get_sub_status();
		$data['status'] = $objBreakdown->get_status();

        return view('reports.breakdown_report')->with('breakdown', $data);
    }

	public function breakdown_report_process(Request $request){
		...
		$tid_filter = '';
		$fault_serial_no_filter = '';
		$replacement_serial_no_filter = '';
		$bank_filter = '';
		$model_filter = '';
		$fault_filter = '';
		$relevant_detail_filter = '';
		$action_taken_filter = '';
		$officer_filter = '';
		$sub_status_filter = '';
		$status_filter = '';
		...
		//Fault Serial No.
		if( isset($input['from_fault_serialno']) && isset($input['to_fault_serialno']) ){

			$fault_serial_no_filter = " && b.fault_serialno between '". $input['from_fault_serialno'] ."' and '". $input['to_fault_serialno'] ."'  ";
		}

		//Replaced Serial No.
		if( isset($input['from_replaced_serialno']) && isset($input['to_replaced_serialno']) ){

			$replacement_serial_no_filter = " && b.replaced_serialno between '". $input['from_replaced_serialno'] ."' and '". $input['to_replaced_serialno'] ."'  ";
		}
		...
		if(isset($input['fault'])){
			...
			$fault_filter = " && b.error in (". $fault_list .") ";
		}


		// Relevant Detail
		if(isset($input['relevant_detail'])){

			$relevant_detail_array = $input['relevant_detail'];
			$relevant_detail_list = '';
			foreach($relevant_detail_array as $relevant_detail){

				$relevant_detail_list .= "'" . $relevant_detail . "', ";
			}
			$relevant_detail_list = substr(rtrim($relevant_detail_list),0,-1);
			$relevant_detail_filter = " && b.relevant_detail in (". $relevant_detail_list .") ";
		}
		...
		if(isset($input['action_taken'])){
			...
			$action_taken_array = $input['action_taken'];
			$action_taken_list = '';
			foreach($action_taken_array as $action_taken){

				$action_taken_list .= "'" . $action_taken . "', ";
			}
			$action_taken_list = substr(rtrim($action_taken_list),0,-1);
			$action_taken_filter = " && b.action_taken in (". $action_taken_list .") ";
		}
		...
		$total_filter = $tid_filter . $fault_serial_no_filter . $replacement_serial_no_filter . $bank_filter . $model_filter . $fault_filter;
		$total_filter .= $relevant_detail_filter . $action_taken_filter . $officer_filter . $sub_status_filter . $status_filter;

		$result = $objBreakdown->get_report($input['from_date'], $input['to_date'], $total_filter, $tables);
	...
	private function genarate_excel_file($resultset, $tables){
		...
		$sheet->setCellValue($this->getExcelColumn($col_count) . '2', 'CC Merchant'); $col_count++;
		$sheet->setCellValue($this->getExcelColumn($col_count) . '2', 'Fault'); $col_count++;
		$sheet->setCellValue($this->getExcelColumn($col_count) . '2', 'Model'); $col_count++;
		$sheet->setCellValue($this->getExcelColumn($col_count) . '2', 'Fault Serial No.'); $col_count++;
		$sheet->setCellValue($this->getExcelColumn($col_count) . '2', 'Replaced Serial No.'); $col_count++;
		...
		$sheet->setCellValue($this->getExcelColumn($col_count) . '2', 'CC Officer'); $col_count++;

		$sheet->setCellValue($this->getExcelColumn($col_count) . '2', 'Caller'); $col_count++;
		$sheet->setCellValue($this->getExcelColumn($col_count) . '2', 'Receiver'); $col_count++;
		$sheet->setCellValue($this->getExcelColumn($col_count) . '2', 'Replacement'); $col_count++;
		$sheet->setCellValue($this->getExcelColumn($col_count) . '2', 'Replacement Ref No.'); $col_count++;
		...
		$sheet->setCellValue($this->getExcelColumn($col_count) . '2', 'CC Relevant Detail'); $col_count++;
		$sheet->setCellValue($this->getExcelColumn($col_count) . '2', 'CC Action Taken'); $col_count++;
		$sheet->setCellValue($this->getExcelColumn($col_count) . '2', 'CC Remark'); $col_count++;
		$sheet->setCellValue($this->getExcelColumn($col_count) . '2', 'Call To Merchant'); $col_count++;
		...
		foreach($resultset as $row){
			...
			$sheet->setCellValue($this->getExcelColumn($col_count) . $icount, $row->cc_merchant);  $col_count++;
			$sheet->setCellValue($this->getExcelColumn($col_count) . $icount, $row->fault);  $col_count++;
			$sheet->setCellValue($this->getExcelColumn($col_count) . $icount, $row->model);  $col_count++;
			$sheet->setCellValue($this->getExcelColumn($col_count) . $icount, $row->cc_fault_serialno);  $col_count++;
			$sheet->setCellValue($this->getExcelColumn($col_count) . $icount, $row->cc_replaced_serialno);  $col_count++;
			$sheet->setCellValue($this->getExcelColumn($col_count) . $icount, $row->cc_contactno);  $col_count++;
			$sheet->setCellValue($this->getExcelColumn($col_count) . $icount, $row->cc_contact_person);  $col_count++;
			$sheet->setCellValue($this->getExcelColumn($col_count) . $icount, $row->cc_officer);  $col_count++;
			$sheet->setCellValue($this->getExcelColumn($col_count) . $icount, $row->caller);  $col_count++;

			$sheet->setCellValue($this->getExcelColumn($col_count) . $icount, $row->receiver);  $col_count++;
			$sheet->setCellValue($this->getExcelColumn($col_count) . $icount, $this->YesNo($row->replacement));  $col_count++;
			$sheet->setCellValue($this->getExcelColumn($col_count) . $icount, $row->replacement_refno);  $col_count++;
			$sheet->setCellValue($this->getExcelColumn($col_count) . $icount, $row->courier);  $col_count++;
			$sheet->setCellValue($this->getExcelColumn($col_count) . $icount, $row->field_officer);  $col_count++;
			$sheet->setCellValue($this->getExcelColumn($col_count) . $icount, $row->officer_name);  $col_count++;
			$sheet->setCellValue($this->getExcelColumn($col_count) . $icount, $row->cc_relevant_detail);  $col_count++;
			$sheet->setCellValue($this->getExcelColumn($col_count) . $icount, $row->cc_action_taken);  $col_count++;
			$sheet->setCellValue($this->getExcelColumn($col_count) . $icount, $row->cc_remark);  $col_count++;
			$sheet->setCellValue($this->getExcelColumn($col_count) . $icount, $this->YesNo($row->call_to_merchant));  $col_count++;
			...
			$sheet->setCellValue($this->getExcelColumn($col_count) . $icount, $row->cc_last_edit_by);  $col_count++;
			$sheet->setCellValue($this->getExcelColumn($col_count) . $icount, $row->cc_last_edit_on);  $col_count++;
	...
	function get_tmc_item($ticketno){


	}

	function get_field_service_faults($ticketno){


	}

	function get_field_service_action_taken($ticketno){
