...
namespace App\Http\Controllers\Tmc\Backup\Note;
...
class BackupRemoveNoteController extends Controller {
	...
	public function index(){
		...
		$objCourier = new CourierProvider();
		...
		$data['attributes'] = $this->getBackupRemoveNoteAttributes(NULL, NULL);

		return view('tmc.backup.note.backup_remove_note')->with('BRN', $data);
	}

	private function getBackupRemoveNoteAttributes($process, $request){
        ...
        $attributes['received_serialno'] = '';
        $attributes['received_model'] = 0;
		$attributes['remark'] = '';
		$attributes['fs_remark'] = '';
		$attributes['contact_number'] = '';
		$attributes['contact_person'] = '';
		$attributes['officer'] = '';
		$attributes['courier'] = '';
		$attributes['bank_officer'] = '';
		$attributes['sub_status'] = 6;
		$attributes['status'] = 'inprogress';
		...
		if( ($process['validation_result'] == TRUE) && ($process['process_status'] == TRUE)){
			...
			foreach($brn_resultset as $row){
                ...
                $attributes['received_serialno'] = $row->received_serialno;
                $attributes['received_model'] = $row->received_model;
			...
			$brn_fs_result = $objBackupProcess->getBackupRemovalFieldService($process['brn_no']);
			foreach($brn_fs_result as $row){

				$attributes['fs_remark'] = $row->remark;
			}

			$attributes['validation_messages'] = new MessageBag();

			if( $process['validation_result'] == '@@' ){

				$attributes['history'] = $objBackupProcess->get_backup_remove_note_history($process['brn_no']);
            ...
            if(is_null($input) == FALSE){
                ...
                $attributes['received_serialno'] = $input['received_serialno'];
                $attributes['received_model'] = $input['received_model'];
	...
	public function getBackupRemoveNote(Request $request){
		...
		$data['sub_status'] = $objSubStatus->getTmcBackupRemoveSubStatus();
		$data['status'] = $objStatus->getBackupRemoveStatus();
		$data['attributes'] = $this->getBackupRemoveNoteAttributes($process, NULL);
	...
	public function backup_remove_note_process(Request $request){
		...
		if($request->submit == 'Save'){
			...
			if($validation_result['validation_result'] == TRUE){
				...
				$brn_saving_result = $this->savingProcess($request);
				...
				$data['attributes'] = $this->getBackupRemoveNoteAttributes($brn_saving_result, $request);
				...
				$data['attributes'] = $this->getBackupRemoveNoteAttributes($validation_result, $request);
			}
		}
		...
		if($request->submit == 'Cancel'){
			...
			if($validation_result['validation_result'] == TRUE){
				...
				$data['attributes'] = $this->getBackupRemoveNoteAttributes($brn_cancel_result, $request);
	...
	private function backup_remove_note($request){
        ...
        $brn_array['received_serialno'] = $request->received_serialno;
		$brn_array['received_model'] = $request->received_model;
	...
	private function backup_remove_note_history($request){
        ...
        $new_array['Received Serialno'] = $request->received_serialno;
		$new_array['Received Model'] = $request->received_model;
		...
		$new_array['Sub Status'] = $objBackupProcess->getBackupRemovalSubStatusDescription($request->sub_status);
		...
		if(count($result) >= 1){
			...
			foreach($result as $row){
                ...
                $old_array['Received Serialno'] = $row->received_serialno;
				$old_array['Received Model'] = $row->received_model;
				$old_array['Contact Number'] = $row->contact_number;
				$old_array['Contact Person'] = $row->contact_person;
				$old_array['Remark'] = $row->remark;
				$old_array['Sub Status'] = $objBackupProcess->getBackupRemovalSubStatusDescription($row->sub_status);
            ...
            $old_array['Received Serialno'] = '';
			$old_array['Received Model'] = '';
	...
	public function get_merchant_detail(Request $request){
		...
		// $objVericentreController = new VericentreController();
		// $merchant_detail = $objVericentreController->getMerchantDetail($request->terminal_id);

		// echo $merchant_detail['merchant'] . chr(13) . $merchant_detail['address_line_one'] . chr(13) . $merchant_detail['address_line_two'];

        $objBackupProcess = new BackupProcess();
        $merchant = '';
        $contact_number = '';

		$last_tid_rocord = $objBackupProcess->getLastTerminalIdRecord($request->terminal_id);
        foreach($last_tid_rocord as $row){
