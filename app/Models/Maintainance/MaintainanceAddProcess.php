...
class MaintainanceAddProcess extends Model {

    use HasFactory;

    public function save_maintainance_add_notes($data){

        DB::beginTransaction();

		try{

			$maintainance_add_notes = $data['maintainance_add_note'];
            $terminal_log = $data['terminal_log'];

			foreach($maintainance_add_notes as $row){

				DB::table('maintainance_add_note')->insert($row);
    ...
    public function get_maintainance_add_inquire_process($query_string){

		$sql_query = " select		ma_id, ma_date, bank, mc.mc_name, referance_number, remark
                       from		    maintainance_add_note man 
                                        inner join maintainance_category mc on man.maintainance_category_id = mc.mc_id
