...
<?php
    ...
    if(isset($software_updation['selected_date'])){

        $from_date = $software_updation['selected_date'][0];
        $to_date = $software_updation['selected_date'][1];
    }
...
@section('body')
            ...
            <h3> Software Updation Workflow </h3>
            <hr>

            <form method="POST" action="{{route('software_updation_management_process')}}">
                ...
                <div class="row">
                    ...
                    <div class="col-sm-4">
                        <div class="card bg-light mb-3" style="max-width: 18rem;">
                            <div class="card-header">Total Software Updation Count</div>
                            <div class="card-body">
                                <h2 class="card-title" style="text-align: center;">
                                    @if( isset($software_updation['software_updation_count']) )
                                        @foreach($software_updation['software_updation_count'] as $row)
                                            {{$row->software_updation_count}}
                                        @endforeach
                                    @else
                                        0
                                    @endif
                        ...
                        <div class="card">
                            <div class="card-body">
                                <table class="table table-hover table-sm table-bordered" style='font-family: Consolas; font-size: 13px;'>
                                    ...
                                    @if( isset($software_updation['software_updation_bank_count']) )
                                        @if(count($software_updation['software_updation_status_count']) >= 1)
                                            @foreach($software_updation['software_updation_bank_count'] as $row)
                        ...
                        <div class="card">
                            <div class="card-body">
                                <table class="table table-hover table-sm table-bordered" style='font-family: Consolas; font-size: 13px;'>
                                    ...
                                    @if( isset($software_updation['software_updation_status_count']) )
                                        @if(count($software_updation['software_updation_status_count']) >= 1)
                                            @foreach($software_updation['software_updation_status_count'] as $row)
...
<script>
    ...
    function getBank(bank){
        ...
        $.ajax({
            ...
            url:  "{{ route('get_bank_wise_status_wise_software_updation_management_count') }}",
            type: 'GET',
    ...
    function getStatus(status){
        ...
        $.ajax({
            ...
            url:  "{{ route('get_status_wise_bank_wise_software_updation_management_count') }}",
            type: 'GET',
    ...
    function detail(text){
        ...
        $.ajax({
            ...
            url:  "{{route('get_software_updation_management_detail')}}",
            type: 'GET',
            ...
            success: function(data) {
                ...
                if((Object.keys(document_table).length) >= 1){
                    ...
                    for (var i = 0; i < (Object.keys(document_table).length); i++) {
                        ...
                        software_updation_officer = document_table[i]['officer_name'];
                        if(software_updation_officer == null){
                            software_updation_officer = '-';
                        }
                        ...
                        software_updation_status = document_table[i]['status'];
                        software_updation_status = (software_updation_status.charAt(0).toUpperCase() + software_updation_status.slice(1));
                        ...
                        row_detail += "<td >" + software_updation_officer + "</td>";
                        row_detail += "<td >" + software_updation_status  + "</td>";
