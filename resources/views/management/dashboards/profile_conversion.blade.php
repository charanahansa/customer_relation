...
<?php
    ...
    if(isset($profile_conversion['selected_date'])){

        $from_date = $profile_conversion['selected_date'][0];
        $to_date = $profile_conversion['selected_date'][1];
    }
...
@section('body')
            ...
            <h3> Profile Conversion Workflow </h3>
            <hr>

            <form method="POST" action="{{route('profile_conversion_management_process')}}">
                ...
                <div class="row">
                    ...
                    <div class="col-sm-4">
                        <div class="card bg-light mb-3" style="max-width: 18rem;">
                            <div class="card-header">Total Profile Conversion Count</div>
                            <div class="card-body">
                                <h2 class="card-title" style="text-align: center;">
                                    @if( isset($profile_conversion['profile_conversion_count']) )
                                        @foreach($profile_conversion['profile_conversion_count'] as $row)
                                            {{$row->profile_conversion_count}}
                                        @endforeach
                                    @else
                                        0
                                    @endif
                        ...
                        <div class="card">
                            <div class="card-body">
                                <table class="table table-hover table-sm table-bordered" style='font-family: Consolas; font-size: 13px;'>
                                    ...
                                    @if( isset($profile_conversion['profile_conversion_bank_count']) )
                                        @if(count($profile_conversion['profile_conversion_status_count']) >= 1)
                                            @foreach($profile_conversion['profile_conversion_bank_count'] as $row)
                        ...
                        <div class="card">
                            <div class="card-body">
                                <table class="table table-hover table-sm table-bordered" style='font-family: Consolas; font-size: 13px;'>
                                    ...
                                    @if( isset($profile_conversion['profile_conversion_status_count']) )
                                        @if(count($profile_conversion['profile_conversion_status_count']) >= 1)
                                            @foreach($profile_conversion['profile_conversion_status_count'] as $row)
...
<script>
    ...
    function getBank(bank){
        ...
        $.ajax({
            ...
            url:  "{{ route('get_bank_wise_status_wise_profile_conversion_management_count') }}",
            type: 'GET',
    ...
    function getStatus(status){
        ...
        $.ajax({
            ...
            url:  "{{ route('get_status_wise_bank_wise_profile_conversion_management_count') }}",
            type: 'GET',
    ...
    function detail(text){
        ...
        $.ajax({
            ...
            url:  "{{route('get_profile_conversion_management_detail')}}",
            type: 'GET',
            ...
            success: function(data) {
                ...
                myObj = JSON.parse(data);
                document_table = myObj['profile_conversion_detail'];
                ...
                if((Object.keys(document_table).length) >= 1){
                    ...
                    for (var i = 0; i < (Object.keys(document_table).length); i++) {
                        ...
                        profile_conversion_officer = document_table[i]['officer_name'];
                        if(profile_conversion_officer == null){
                            profile_conversion_officer = '-';
                        }

                        profile_conversion_status = document_table[i]['status'];
                        profile_conversion_status = (profile_conversion_status.charAt(0).toUpperCase() + profile_conversion_status.slice(1));
                        ...
                        row_detail += "<td >" + profile_conversion_officer + "</td>";
                        row_detail += "<td >" + profile_conversion_status  + "</td>";
                        row_detail +=  "</tr>";
