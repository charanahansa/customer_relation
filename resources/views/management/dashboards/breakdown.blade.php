...
<?php
    ...
    if(isset($breakdown['selected_date'])){

        $from_date = $breakdown['selected_date'][0];
        $to_date = $breakdown['selected_date'][1];
    }
?>

@section('body')

    <h3> Breakdown Workflow </h3>
    <hr>
    <div style = "display: block; text-align: center;">

        <form method="POST" name="form_name" action="{{route('breakdown_management_process')}}" style = " margin-left: auto; margin-right: auto; text-align: left;">
                        ...
                        <div class="card bg-light mb-3" style="max-width: 18rem;">
                            ...
                            <div class="card-body">
                                <h2 class="card-title" style="text-align: center;">
                                    @if( isset($breakdown['breakdown_count']) )
                                        @foreach($breakdown['breakdown_count'] as $row)
                                            {{$row->breakdown_count}}
                                        @endforeach
                                    @else
                                        0
                            ...
                            <div class="card-body">
								<div class="table-responsive">
	                                <table class="table table-striped  table-sm table-bordered" style='font-family: Consolas; font-size: 13px;'>
	                                    ...
	                                    @if( isset($breakdown['breakdown_bank_count']) )
	                                        @if(count($breakdown['breakdown_status_count']) >= 1)
	                                            @foreach($breakdown['breakdown_bank_count'] as $row)
                            ...
                            <div class="card-body">
								<div class="table-responsive">
	                                <table class="table table-striped  table-sm table-bordered" style='font-family: Consolas; font-size: 13px;'>
	                                    ...
	                                    @if( isset($breakdown['breakdown_status_count']) )
	                                        @if(count($breakdown['breakdown_status_count']) >= 1)
	                                            @foreach($breakdown['breakdown_status_count'] as $row)
...
<script>
    ...
    function getBank(bank){
        ...
        $.ajax({
            ...
            url:  "{{ route('get_bank_wise_status_wise_breakdown_management_count') }}",
            type: 'GET',
    ...
    function getStatus(status){
        ...
        $.ajax({
            ...
            url:  "{{ route('get_status_wise_bank_wise_breakdown_management_count') }}",
            type: 'GET',
    ...
    function detail(text){
        ...
        $.ajax({
            ...
            url:  "{{route('get_breakdown_management_detail')}}",
            type: 'GET',
            ...
            success: function(data) {
                ...
                if((Object.keys(document_table).length) >= 1){
                    ...
                    for (var i = 0; i < (Object.keys(document_table).length); i++) {
                        ...
                        breakdown_officer = document_table[i]['officer_name'];
                        if(breakdown_officer == null){
                            breakdown_officer = '-';
                        }
                        ...
                        breakdown_status = document_table[i]['status'];
                        breakdown_status = (breakdown_status.charAt(0).toUpperCase() + breakdown_status.slice(1));
						...
						row_detail += "<td >" + document_table[i]['error'] + "</td>";
                        row_detail += "<td >" + document_table[i]['merchant'] + "</td>";
                        row_detail += "<td >" + breakdown_officer + "</td>";
                        row_detail += "<td >" + breakdown_status  + "</td>";
