...
<?php
    ...
    if(isset($re_initialization['selected_date'])){

        $from_date = $re_initialization['selected_date'][0];
        $to_date = $re_initialization['selected_date'][1];
    }
...
@section('body')
            ...
            <h3> Re Initialization Workflow </h3>
            <hr>

            <form method="POST" action="{{route('re_initialization_management_process')}}">
                ...
                <div class="row">
                    ...
                    <div class="col-sm-5">
                        <div class="card bg-light mb-5" style="max-width: 18rem;">
                            <div class="card-header">Total Re Initialization Count</div>
                            <div class="card-body">
                                <h2 class="card-title" style="text-align: center;">
                                    @if( isset($re_initialization['re_initialization_count']) )
                                        @foreach($re_initialization['re_initialization_count'] as $row)
                                            {{$row->re_initialization_count}}
                                        @endforeach
                                    @else
                                        0
                                    @endif
                        ...
                        <div class="card">
                            <div class="card-body">
                                <table class="table table-hover table-sm table-bordered" style='font-family: Consolas; font-size: 13px;'>
                                    ...
                                    @if( isset($re_initialization['re_initialization_bank_count']) )
                                        @if(count($re_initialization['re_initialization_status_count']) >= 1)
                                            @foreach($re_initialization['re_initialization_bank_count'] as $row)
                        ...
                        <div class="card">
                            <div class="card-body">
                                <table class="table table-hover table-sm table-bordered" style='font-family: Consolas; font-size: 13px;'>
                                    ...
                                    @if( isset($re_initialization['re_initialization_status_count']) )
                                        @if(count($re_initialization['re_initialization_status_count']) >= 1)
                                            @foreach($re_initialization['re_initialization_status_count'] as $row)
...
<script>
    ...
    function getBank(bank){
        ...
        $.ajax({
            ...
            url:  "{{ route('get_bank_wise_status_wise_re_initialization_management_count') }}",
            type: 'GET',
    ...
    function getStatus(status){
        ...
        $.ajax({
            ...
            url:  "{{ route('get_status_wise_bank_wise_re_initialization_management_count') }}",
            type: 'GET',
    ...
    function detail(text){
        ...
        $.ajax({
            ...
            url:  "{{route('get_re_initialization_management_detail')}}",
            type: 'GET',
            ...
            success: function(data) {
                ...
                myObj = JSON.parse(data);
                document_table = myObj['re_initialization_detail'];

                // Header
                var row_header = "<tr class='table-info'>";
                row_header += "<td style='width: 3%;'>No.</td>";
                row_header += "<td style='width: 5%;'>Ticket No.</td>";
                row_header += "<td style='width: 10%;'>Date & Time</td>";
                row_header += "<td style='width: 5%;'>Bank</td>";
                row_header += "<td style='width: 7%;'>Tid</td>";
                row_header += "<td style='width: 7%;'>Model</td>";
                row_header += "<td style='width: 30%;'>Merchant</td>";
                row_header += "<td style='width: 15%;'>Field Officer</td>";
                row_header += "<td style='width: 9%;'>status</td>";
                row_header += "<td style='width: 14%;'>Done Date Time</td>";
                row_header +=  "</tr>";
                ...
                if((Object.keys(document_table).length) >= 1){
                    ...
                    for (var i = 0; i < (Object.keys(document_table).length); i++) {
                        ...
                        re_initialization_officer = document_table[i]['officer_name'];
                        if(re_initialization_officer == null){
                            re_initialization_officer = '-';
                        }
                        ...
                        re_initialization_status = document_table[i]['status'];
                        re_initialization_status = (re_initialization_status.charAt(0).toUpperCase() + re_initialization_status.slice(1));
                        ...
                        row_detail += "<td >" + re_initialization_officer + "</td>";
                        row_detail += "<td >" + re_initialization_status  + "</td>";
