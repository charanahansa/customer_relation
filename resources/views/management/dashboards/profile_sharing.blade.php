...
<?php
    ...
    if(isset($profile_sharing['selected_date'])){

        $from_date = $profile_sharing['selected_date'][0];
        $to_date = $profile_sharing['selected_date'][1];
    }
...
@section('body')
            ...
            <h3> Profile Sharing Workflow </h3>
            <hr>

            <form method="POST" action="{{route('profile_sharing_management_process')}}">
                ...
                <div class="row">
                    ...
                    <div class="col-sm-4">
                        <div class="card bg-light mb-3" style="max-width: 18rem;">
                            <div class="card-header">Total Breakdown Count</div>
                            <div class="card-body">
                                <h2 class="card-title" style="text-align: center;">
                                    @if( isset($profile_sharing['profile_sharing_count']) )
                                        @foreach($profile_sharing['profile_sharing_count'] as $row)
                                            {{$row->profile_sharing_count}}
                                        @endforeach
                                    @else
                                        0
                                    @endif
                        ...
                        <div class="card">
                            <div class="card-body">
                                <table class="table table-hover table-sm table-bordered" style='font-family: Consolas; font-size: 13px;'>
                                    ...
                                    @if( isset($profile_sharing['profile_sharing_bank_count']) )
                                        @if(count($profile_sharing['profile_sharing_status_count']) >= 1)
                                            @foreach($profile_sharing['profile_sharing_bank_count'] as $row)
                        ...
                        <div class="card">
                            <div class="card-body">
                                <table class="table table-hover table-sm table-bordered" style='font-family: Consolas; font-size: 13px;'>
                                    ...
                                    @if( isset($profile_sharing['profile_sharing_status_count']) )
                                        @if(count($profile_sharing['profile_sharing_status_count']) >= 1)
                                            @foreach($profile_sharing['profile_sharing_status_count'] as $row)
...
<script>
    ...
    function getBank(bank){
        ...
        $.ajax({
            ...
            url:  "{{ route('get_bank_wise_status_wise_profile_sharing_management_count') }}",
            type: 'GET',
    ...
    function getStatus(status){
        ...
        $.ajax({
            ...
            url:  "{{ route('get_status_wise_bank_wise_profile_sharing_management_count') }}",
            type: 'GET',
    ...
    function detail(text){
        ...
        $.ajax({
            ...
            url:  "{{route('get_profile_sharing_management_detail')}}",
            type: 'GET',
            ...
            success: function(data) {
                ...
                myObj = JSON.parse(data);
                document_table = myObj['profile_sharing_detail'];
                ...
                row_header += "<td style='width: 10%;'>Sharing Tid</td>";
                ...
                if((Object.keys(document_table).length) >= 1){
                    ...
                    for (var i = 0; i < (Object.keys(document_table).length); i++) {
                        ...
                        profile_sharing_officer = document_table[i]['officer_name'];
                        if(profile_sharing_officer == null){
                            profile_sharing_officer = '-';
                        }

                        profile_sharing_status = document_table[i]['status'];
                        profile_sharing_status = (profile_sharing_status.charAt(0).toUpperCase() + profile_sharing_status.slice(1));
                        ...
                        row_detail += "<td >" + document_table[i]['sharing_tid'] + "</td>";
                        row_detail += "<td >" + document_table[i]['merchant'] + "</td>";
                        row_detail += "<td >" + profile_sharing_officer + "</td>";
                        row_detail += "<td >" + profile_sharing_status  + "</td>";
                        row_detail +=  "</tr>";
