...
<?php
    ...
    if(isset($terminal_replacement['selected_date'])){

        $from_date = $terminal_replacement['selected_date'][0];
        $to_date = $terminal_replacement['selected_date'][1];
    }
?>

@section('body')

            <h3> Terminal Replacement Workflow </h3>
            <hr>

            <form method="POST" action="{{route('terminal_replacement_management_process')}}">
                ...
                <div class="row">
                    ...
                    <div class="col-sm-4">
                        <div class="card bg-light mb-3" style="max-width: 18rem;">
                            <div class="card-header">Total Terminal Replacement Count</div>
                            <div class="card-body">
                                <h2 class="card-title" style="text-align: center;">
                                    @if( isset($terminal_replacement['terminal_replacement_count']) )
                                        @foreach($terminal_replacement['terminal_replacement_count'] as $row)
                                            {{$row->terminal_replacement_count}}
                                        @endforeach
                                    @else
                                        0
                                    @endif
                        ...
                        <div class="card">
                            <div class="card-body">
                                <table class="table table-hover table-sm table-bordered" style='font-family: Consolas; font-size: 13px;'>
                                    ...
                                    @if( isset($terminal_replacement['terminal_replacement_bank_count']) )
                                        @if(count($terminal_replacement['terminal_replacement_status_count']) >= 1)
                                            @foreach($terminal_replacement['terminal_replacement_bank_count'] as $row)
                        ...
                        <div class="card">
                            <div class="card-body">
                                <table class="table table-hover table-sm table-bordered" style='font-family: Consolas; font-size: 13px;'>
                                    ...
                                    @if( isset($terminal_replacement['terminal_replacement_status_count']) )
                                        @if(count($terminal_replacement['terminal_replacement_status_count']) >= 1)
                                            @foreach($terminal_replacement['terminal_replacement_status_count'] as $row)
...
<script>
    ...
    function getBank(bank){
        ...
        $.ajax({
            ...
            url:  "{{ route('get_bank_wise_status_wise_terminal_replacement_management_count') }}",
            type: 'GET',
    ...
    function getStatus(status){
        ...
        $.ajax({
            ...
            url:  "{{ route('get_status_wise_bank_wise_terminal_replacement_management_count') }}",
            type: 'GET',
    ...
    function detail(text){
        ...
        $.ajax({
            ...
            url:  "{{route('get_terminal_replacement_management_detail')}}",
            type: 'GET',
            ...
            success: function(data) {
                ...
                myObj = JSON.parse(data);
                document_table = myObj['terminal_replacement_detail'];
                ...
                row_header += "<td style='width: 10%;'>Done Date Time</td>";
                row_header +=  "</tr>";
                ...
                if((Object.keys(document_table).length) >= 1){
                    ...
                    for (var i = 0; i < (Object.keys(document_table).length); i++) {
                        ...
                        terminal_replacement_officer = document_table[i]['officer_name'];
                        if(terminal_replacement_officer == null){
                            terminal_replacement_officer = '-';
                        }
                        ...
                        terminal_replacement_status = document_table[i]['status'];
                        terminal_replacement_status = (terminal_replacement_status.charAt(0).toUpperCase() + terminal_replacement_status.slice(1));
                        ...
                        row_detail += "<td >" + terminal_replacement_officer + "</td>";
                        row_detail += "<td >" + terminal_replacement_status  + "</td>";
