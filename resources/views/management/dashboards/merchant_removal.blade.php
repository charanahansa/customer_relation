...
<?php
    ...
    if(isset($merchant_removal['selected_date'])){

        $from_date = $merchant_removal['selected_date'][0];
        $to_date = $merchant_removal['selected_date'][1];
    }
?>

@section('body')

            <h3> Merchant Removal Workflow </h3>
            <hr>

            <form method="POST" action="{{route('merchant_removal_management_process')}}">
                ...
                <div class="row">
                    ...
                    <div class="col-sm-4">
                        <div class="card bg-light mb-3" style="max-width: 18rem;">
                            <div class="card-header">Total Merchant Removal Count</div>
                            <div class="card-body">
                                <h2 class="card-title" style="text-align: center;">
                                    @if( isset($merchant_removal['merchant_removal_count']) )
                                        @foreach($merchant_removal['merchant_removal_count'] as $row)
                                            {{$row->merchant_removal_count}}
                                        @endforeach
                                    @else
                                        0
                                    @endif
                        ...
                        <div class="card">
                            <div class="card-body">
                                <table class="table table-hover table-sm table-bordered" style='font-family: Consolas; font-size: 13px;'>
                                    ...
                                    @if( isset($merchant_removal['merchant_removal_bank_count']) )
                                        @if(count($merchant_removal['merchant_removal_status_count']) >= 1)
                                            @foreach($merchant_removal['merchant_removal_bank_count'] as $row)
                        ...
                        <div class="card">
                            <div class="card-body">
                                <table class="table table-hover table-sm table-bordered" style='font-family: Consolas; font-size: 13px;'>
                                    ...
                                    @if( isset($merchant_removal['merchant_removal_status_count']) )
                                        @if(count($merchant_removal['merchant_removal_status_count']) >= 1)
                                            @foreach($merchant_removal['merchant_removal_status_count'] as $row)
...
<script>
    ...
    function getBank(bank){
        ...
        $.ajax({
            ...
            url:  "{{ route('get_bank_wise_status_wise_merchant_removal_management_count') }}",
            type: 'GET',
    ...
    function getStatus(status){
        ...
        $.ajax({
            ...
            url:  "{{ route('get_status_wise_bank_wise_merchant_removal_management_count') }}",
            type: 'GET',
    ...
    function detail(text){
        ...
        $.ajax({
            ...
            url:  "{{route('get_merchant_removal_management_detail')}}",
            type: 'GET',
            ...
            success: function(data) {
                ...
                myObj = JSON.parse(data);
                document_table = myObj['merchant_removal_detail'];
                ...
                if((Object.keys(document_table).length) >= 1){
                    ...
                    for (var i = 0; i < (Object.keys(document_table).length); i++) {
                        ...
                        merchant_removal_officer = document_table[i]['officer_name'];
                        if(merchant_removal_officer == null){
                            merchant_removal_officer = '-';
                        }

                        merchant_removal_status = document_table[i]['status'];
                        merchant_removal_status = (merchant_removal_status.charAt(0).toUpperCase() + merchant_removal_status.slice(1));
                        ...
                        row_detail += "<td >" + merchant_removal_officer + "</td>";
                        row_detail += "<td >" + merchant_removal_status  + "</td>";
                        row_detail +=  "</tr>";
