...
@section('title')
    Terminal Replacement Inquire
@endsection

@section('body')

<form method="POST" name="form_name" action="{{route('fs_terminal_replacement_inquire_process')}}">
				...
				<div class="table-responsive">
					<div id="tbldiv" style="width: 100%;">
						...
						<table id="tblgrid1" class="table table-hover table-sm table-bordered">
							...
							@if(count($TR['report_table']))

								<tbody>
									@foreach($TR['report_table'] as $row)
										<tr style="font-family: Consolas; font-size: 13px;">
											...
											<td>{{$row->bank}}</td>
											<td>{{$row->based_tid}}</td>
											<td>{{$row->based_serialno}}</td>
											<td>{{$row->based_model}}</td>
											<td>{{$row->replaced_tid}}</td>
...
<script>
    ...
    function openTicket(ticket_number, bank){
        ...
        document.getElementById("workflow_id").value = 5;
